...
export class VideosComponent {
  ...
  renderQueueStatus() {
    ...
    if (failedJobs.length > 0) {
      ...
      failedJobs.forEach(job => {
        html += `
          <div class="queue-job failed-job" style="padding: 16px; border: 1px solid #fecaca; background: #fff1f2; margin-bottom: 10px; border-left: 4px solid #ef4444; border-radius: 10px; position: relative;">
            <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 4px;">
              ...
              <div style="display: flex; align-items: center; gap: 8px;">
                ...
                <button class="icon-btn danger" onclick="window.videosComponent.deleteJobHistory('${job.id}')" title="Delete from history" style="padding: 2px;">
  ...
  async deleteJobHistory(jobId) {
    if (!confirm('Remove this failed job from history?')) return;

    try {
      const res = await fetch(`/api/videos/queue/history/${jobId}`, {
        method: 'DELETE'
      });
      const result = await res.json();
