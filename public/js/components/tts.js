class TTSComponent {
    ...
    return `
      <div class="tts-panel">
        <div class="tts-generate-steps">
          ...
          <!-- Generation History -->
          <div id="ttsHistoryArea" class="tts-history-area" style="margin-top: 30px;">
            <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 15px; border-bottom: 2px solid #eef2ff; padding-bottom: 10px;">
              <h3 style="margin: 0; display: flex; align-items: center; gap: 8px;">
                <span class="material-symbols-outlined">history</span>
                Generation History
              </h3>
              <button class="btn btn-secondary btn-small" id="refreshTTSHistoryBtn">
                <span class="material-symbols-outlined" style="font-size: 16px;">refresh</span>
                Refresh
              </button>
            </div>
            <div id="ttsHistoryList">
              <p style="text-align: center; color: #888; padding: 20px;">No generation history available.</p>
            </div>
          </div>
  ...
  attachGenerateEventListeners() {
    const refreshHistoryBtn = document.getElementById('refreshTTSHistoryBtn');
  ...
  updateHistoryUI() {
    ...
    if (!this.allJobs || this.allJobs.length === 0) {
      historyList.innerHTML = '<p style="text-align: center; color: #888; padding: 20px;">No generation history available.</p>';
      return;
    }

    // Only show completed/failed jobs in history, or jobs that aren't the current active one
    const historyJobs = this.allJobs;

    historyList.innerHTML = historyJobs.map(job => {
      const isRunning = job.status === 'running' || job.status === 'queued';
      const isFailed = job.status === 'failed';
      const isCompleted = job.status === 'completed';

      const statusColor = isRunning ? '#4f46e5' : (isFailed ? '#ef4444' : '#10b981');
      const bgColor = isRunning ? '#f5f3ff' : (isFailed ? '#fff1f2' : '#f0fdf4');
      const borderColor = isRunning ? '#c7d2fe' : (isFailed ? '#fecaca' : '#bbf7d0');

      return `
        <div class="tts-history-item" style="background: ${bgColor}; border: 1px solid ${borderColor}; border-left: 4px solid ${statusColor}; padding: 15px; border-radius: 8px; margin-bottom: 10px;">
          <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 8px;">
            <div style="display: flex; align-items: center; gap: 8px;">
              <span class="material-symbols-outlined" style="color: ${statusColor}; font-size: 20px;">
                ${isRunning ? 'sync' : (isFailed ? 'error' : 'check_circle')}
              </span>
              <strong style="color: #1e293b;">${this.escapeHtml(job.payload?.setName || 'Unnamed Set')}</strong>
            </div>
            <span style="font-size: 12px; color: #64748b;">${new Date(job.created_at).toLocaleString()}</span>
          </div>

          <div style="display: flex; justify-content: space-between; align-items: center; font-size: 13px; color: #475569;">
            <div>
              ${job.progress.completed_items} / ${job.progress.total_items} items processed
              ${job.progress.summary.failed > 0 ? `<span style="color: #ef4444; margin-left: 10px;">(${job.progress.summary.failed} failed)</span>` : ''}
            </div>
            <div style="font-weight: 700; color: ${statusColor}; text-transform: uppercase; font-size: 11px;">
              ${job.status}
            </div>
          </div>

          ${isFailed && job.error ? `
            <div style="margin-top: 8px; padding: 8px; background: rgba(239, 68, 68, 0.1); border-radius: 4px; color: #b91c1c; font-size: 12px;">
          ...
          ${job.progress.errors && job.progress.errors.length > 0 ? `
            <div style="margin-top: 8px;">
              <button class="btn btn-secondary btn-small" style="font-size: 11px; padding: 2px 8px;" onclick="ttsComponent.toggleJobLogs('${job.id}')">
                Toggle Detailed Logs
              </button>
              <div id="logs-${job.id}" style="display: none; margin-top: 8px; max-height: 150px; overflow-y: auto; font-size: 11px; padding: 8px; background: #fff; border: 1px solid #ddd; border-radius: 4px;">
                ${job.progress.errors.map(err => `
                  <div style="margin-bottom: 4px; color: #ef4444;">
                    &bull; <strong>${this.escapeHtml(err.category)}:</strong> ${this.escapeHtml(err.error)}
                  </div>
                `).join('')}
              </div>
            </div>
