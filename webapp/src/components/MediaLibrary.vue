<template>
  <div class="p-6 bg-background-light dark:bg-background-dark font-sans">
    <div class="flex flex-col md:flex-row justify-between md:items-center mb-4 gap-3">
      ...
      <div class="flex flex-col md:flex-row gap-2">
        <button 
          ...
          class="flex items-center justify-center gap-2 px-3 py-2 rounded-md border border-border-light dark:border-border-dark bg-card-light dark:bg-card-dark text-text-light dark:text-text-dark text-sm"
        >
        ...
        <button 
          @click="addFiles" 
          class="flex items-center justify-center gap-2 px-3 py-2 rounded-md bg-primary text-white text-sm"
        >
        ...
        <ol class="inline-flex items-center space-x-1 md:space-x-2 rtl:space-x-reverse">
          <li class="inline-flex items-center">
            <a 
              ...
              class="inline-flex items-center text-xs font-medium text-subtext-light hover:text-primary dark:text-subtext-dark dark:hover:text-white cursor-pointer"
            >
          ...
          <li v-for="(folder, index) in currentPath" :key="index">
            <div class="flex items-center">
              ...
              <a 
                ...
                class="ms-1 text-xs font-medium text-subtext-light hover:text-primary md:ms-2 dark:text-subtext-dark dark:hover:text-white cursor-pointer"
              >
                {{ folder.name }}
              </a>
            </div>
      ...
      <div class="w-full flex-grow flex flex-col sm:flex-row gap-2">
        <div class="relative flex-grow">
          ...
          <input 
            v-model="searchQuery" 
            @input="searchItems"
            class="w-full pl-10 pr-4 py-2 rounded-md border border-border-light dark:border-border-dark bg-card-light dark:bg-card-dark text-text-light dark:text-text-dark focus:ring-primary focus:border-primary text-sm h-full" 
            placeholder="Search" 
            type="text"
          />
        </div>
        <div class="flex gap-2 justify-end">
          <div class="relative" v-if="!searchQuery">
            <button 
              @click="sortOpen = !sortOpen" 
              class="flex items-center gap-2 px-3 py-2 rounded-md border border-border-light dark:border-border-dark bg-card-light dark:bg-card-dark text-text-light dark:text-text-dark text-sm h-full"
            >
            ...
            <div 
              ...
              class="absolute z-10 mt-1 w-56 bg-card-light dark:bg-card-dark rounded-lg shadow-lg border border-border-light dark:border-border-dark"
            >
              <ul class="py-1 text-sm">
                <li>
                  <a 
                    @click="setSort('name', 'asc')" 
                    class="flex items-center justify-between px-3 py-1.5 hover:bg-gray-100 dark:hover:bg-gray-700 text-text-light dark:text-text-dark cursor-pointer"
                  >
                    <span class="flex items-center gap-2">
                      <span class="material-icons text-base">arrow_upward</span> Name
                    </span>
                    <span v-if="sortBy === 'name' && sortOrder === 'asc'" class="material-icons text-lg">check</span>
                  </a>
                </li>
                <li>
                  <a 
                  ...
                  >
                    <span class="flex items-center gap-2">
                      <span class="material-icons text-base">arrow_downward</span> Name
                    </span>
                    <span v-if="sortBy === 'name' && sortOrder === 'desc'" class="material-icons text-lg">check</span>
                  </a>
                </li>
                <li>
                  <a 
                  ...
                  >
                    Date Created
                    <span v-if="sortBy === 'createdAt' && sortOrder === 'asc'" class="material-icons text-lg">check</span>
                  </a>
                </li>
                <li>
                  <a 
                    @click="setSort('size', 'desc')" 
                    class="flex items-center justify-between px-3 py-1.5 hover:bg-gray-100 dark:hover:bg-gray-700 text-text-light dark:text-text-dark cursor-pointer"
                  >
                    File Size
                    <span v-if="sortBy === 'size' && sortOrder === 'desc'" class="material-icons text-lg">check</span>
                  </a>
                </li>
                <li class="bg-gray-200 dark:bg-gray-600">
                  <a 
                    @click="setSort('updatedAt', 'desc')" 
                    class="flex items-center justify-between px-3 py-1.5 font-semibold text-primary dark:text-white cursor-pointer"
                  >
                    Last Modified 
          ...
          <div class="flex items-center border border-border-light dark:border-border-dark rounded-md bg-card-light dark:bg-card-dark h-full">
            <button 
              :class="{ 'bg-gray-200 dark:bg-gray-600 rounded-l-md': view === 'grid' }" 
              @click="view = 'grid'" 
              class="p-2 text-text-light dark:text-text-dark h-full"
            >
              <span class="material-icons text-xl">grid_view</span>
            </button>
            <button 
              :class="{ 'bg-gray-200 dark:bg-gray-600 rounded-r-md': view === 'list' }" 
              @click="view = 'list'" 
              class="p-2 text-text-light dark:text-text-dark h-full"
            >
      ...
      <div class="flex flex-col sm:flex-row justify-between items-center p-2 bg-blue-100 dark:bg-blue-900/30 rounded-lg">
        ...
        <div class="flex gap-1.5 flex-wrap justify-center">
          <button 
            ...
            class="flex items-center gap-1.5 px-2.5 py-1 rounded-md bg-white dark:bg-card-dark text-text-light dark:text-text-dark border border-border-light dark:border-border-dark shadow-sm text-xs"
          >
          ...
          <button 
            ...
            class="flex items-center gap-1.5 px-2.5 py-1 rounded-md bg-red-600 text-white text-xs"
          >
    ...
    <div class="mb-6">
      ...
      <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4" v-if="view === 'grid'">
        <div 
          ...
          :key="folder.id" 
          class="relative group bg-card-light dark:bg-card-dark rounded-lg shadow-around border-2 border-transparent p-2.5 flex items-center gap-3"
          :class="{'bg-blue-50 dark:bg-blue-900/20': selectedItems.includes(folder.id)}"
        >
          <input 
            v-model="selectedItems" 
            :value="folder.id" 
            type="checkbox" 
            class="form-checkbox h-4 w-4 rounded-full text-primary bg-white/50 border-gray-400/70 focus:ring-0 focus:ring-offset-0"
          />
          ...
          <div class="flex-grow">
            <div v-if="renamingFolderId === folder.id">
              <input 
                ...
                @keyup.enter="renameItem(folder.id, renamingName)" 
                @blur="renameItem(folder.id, renamingName)" 
                class="font-semibold text-text-light dark:text-text-dark bg-gray-100 dark:bg-gray-800 rounded-md px-2 py-0.5 border border-primary focus:outline-none focus:ring-1 focus:ring-primary w-full text-sm" 
                type="text" 
                ref="renameInput"
              />
            </div>
      ...
      <div class="bg-card-light dark:bg-card-dark rounded-lg shadow-sm text-sm" v-if="view === 'list'">
        <div class="space-y-px">
          <div 
            v-for="folder in folders" 
            :key="folder.id" 
            class="flex items-center p-2.5 hover:bg-gray-50 dark:hover:bg-gray-800"
            :class="{'bg-blue-50 dark:bg-blue-900/20': selectedItems.includes(folder.id)}"
          >
            <input 
              v-model="selectedItems" 
              :value="folder.id" 
              type="checkbox" 
              class="form-checkbox h-4 w-4 rounded-full text-primary bg-gray-100 border-gray-300 focus:ring-0 focus:ring-offset-0 dark:bg-gray-600 dark:border-gray-500"
            />
    ...
    <div>
      ...
      <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4" v-if="view === 'grid'">
        <div 
          ...
          :key="video.id" 
          class="relative group bg-card-light dark:bg-card-dark rounded-lg shadow-around overflow-hidden border-2 border-transparent"
          :class="{
            'bg-blue-50 dark:bg-blue-900/20': selectedItems.includes(video.id),
            'opacity-60': video.isMissing
          }"
        >
          <input 
            v-model="selectedItems" 
            :value="video.id" 
            type="checkbox" 
            class="form-checkbox h-4 w-4 rounded-full text-primary bg-white/50 border-gray-400/70 focus:ring-0 focus:ring-offset-0 absolute top-2.5 left-2.5 z-10"
          />
          <div class="aspect-video bg-gray-200 dark:bg-gray-700 flex items-center justify-center relative">
            <span 
              class="material-icons text-gray-400 dark:text-gray-500 text-4xl opacity-50 group-hover:opacity-20 transition-opacity"
              :class="{'text-blue-500 dark:text-blue-400': !video.isMissing}"
            >
              {{ video.isMissing ? 'error_outline' : 'movie' }}
            </span>
            <div 
              v-if="!video.isMissing" 
            ...
            <span 
              v-if="!video.isMissing && video.dimensions" 
              class="absolute bottom-1.5 right-1.5 bg-black/50 text-white text-xs px-1.5 py-0.5 rounded"
            >
              {{ video.dimensions }}
            </span>
          </div>
          <div class="p-2.5">
            <div v-if="renamingVideoId === video.id">
              <input 
                v-model="renamingName" 
                @keyup.enter="renameItem(video.id, renamingName)" 
                @blur="renameItem(video.id, renamingName)" 
                class="font-semibold text-text-light dark:text-text-dark bg-gray-100 dark:bg-gray-800 rounded-md px-2 py-0.5 border border-primary focus:outline-none focus:ring-1 focus:ring-primary w-full text-sm" 
                type="text" 
                ref="renameInput"
              />
            ...
            <span 
              v-if="video.isMissing" 
              class="text-red-500 dark:text-red-400 text-xs font-semibold mt-1 inline-block"
            >
              Source Missing
            </span>
          </div>
        ...
        <table class="w-full text-left text-subtext-light dark:text-subtext-dark">
          ...
          <tbody>
            <tr 
              v-for="video in videos" 
              :key="video.id" 
              class="border-t dark:border-border-dark hover:bg-gray-50 dark:hover:bg-gray-800"
              :class="{'bg-blue-50 dark:bg-blue-900/20': selectedItems.includes(video.id)}"
            >
              <td class="p-3">
                <input 
                  v-model="selectedItems" 
              ...
              <td class="p-3 font-medium text-text-light dark:text-text-dark">
                <div class="flex items-center gap-3">
                  <div class="relative group w-16 h-10 rounded-md overflow-hidden bg-gray-200 dark:bg-gray-700 flex-shrink-0">
                    <span 
                      class="material-icons text-gray-400 dark:text-gray-500 text-2xl absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 group-hover:opacity-20 transition-opacity"
                      :class="{'text-blue-500 dark:text-blue-400': !video.isMissing}"
                    >
                      {{ video.isMissing ? 'error_outline' : 'movie' }}
                    </span>
              ...
              <td class="p-3 text-right">
                <span 
                  v-if="video.isMissing" 
                  class="text-red-500 dark:text-red-400 text-xs font-semibold bg-red-100 dark:bg-red-900/30 px-2 py-1 rounded-full"
                >
                  Source Missing
                </span>
              </td>
    ...
    <!-- Move/Copy Modal -->
    <div 
      v-if="showMoveModal || showCopyModal" 
        ...
        <div class="inline-block align-bottom bg-card-light dark:bg-card-dark rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full">
              ...
              <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left w-full">
                ...
                <div class="mt-4">
                  ...
                  <div class="border border-border-light dark:border-border-dark rounded-lg p-3 space-y-2 h-48 overflow-y-auto">
                    <a 
                      @click="selectDestinationFolder(null)" 
                      :class="{'bg-blue-100 dark:bg-blue-900/40': selectedDestinationFolder === null}"
                    ...
                    <a 
                      @click="selectDestinationFolder(folder.id)" 
                      v-for="folder in allFolders" 
                      :key="folder.id"
                      :class="{'bg-blue-100 dark:bg-blue-900/40': selectedDestinationFolder === folder.id, 'ml-6': folder.parentId}"
                      class="flex items-center p-2 rounded-md hover:bg-gray-100 dark:hover:bg-gray-700 cursor-pointer"
                    >
                      <span class="material-icons text-subtext-light dark:text-subtext-dark mr-2">folder</span>
          ...
          <div class="bg-background-light dark:bg-background-dark px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse">
            <button 
              ...
              :disabled="selectedDestinationFolder === null"
              class="w-full inline-flex justify-center rounded-md border border-transparent shadow-sm px-4 py-2 bg-primary text-base font-medium text-white hover:bg-blue-800 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary sm:ml-3 sm:w-auto sm:text-sm"
              :class="{'opacity-50 cursor-not-allowed': selectedDestinationFolder === null}"
            >
    ...
    <div 
      v-if="toast.message" 
      class="fixed top-5 right-5 z-50 py-3 px-6 rounded-lg shadow-lg flex items-center"
      :class="{'bg-green-500 text-white': toast.type === 'success', 'bg-red-500 text-white': toast.type === 'error'}"
    >
    ...
    >
      <div class="bg-card-dark rounded-lg shadow-2xl w-full max-w-4xl relative" @click.stop>
        <div class="aspect-video">
          ...
          >
            <source 
              :src="`${API_BASE_URL}/api/stream/video/${currentVideo.id}`" 
    ...
    async addFiles() {
      try {
        ...
        input.onchange = async (e) => {
          ...
          if (files.length > 0) {
            // In a real implementation, you would get the file paths
            // For demo purposes, we're showing an alert
            alert(`Selected ${files.length} files. In a real application, these would be added to your media library.`);
          }
        };
