...
<body>
  ...
  async muxTTSAudioIntoVideo({ videoPath, preset, questions, totalDurationSeconds, customName }) {
    ...
    audioInputs.forEach((a, idx) => {
      ...
      // Also apply a volume boost (3.0x) to each segment to address "low audio" issue
      filterParts.push(`[${inLabel}]aresample=44100,aformat=channel_layouts=stereo,volume=3.0,adelay=${delayMs}|${delayMs},apad,atrim=0:${(Number(totalDurationSeconds || 0)).toFixed(3)}[${outLabel}]`);
    });

    const mixInputs = ['[base]', ...audioInputs.map((_, idx) => `[a${idx + 1}]`)].join('');
    // Use volume boost on final mix as well to ensure clear audio
    filterParts.push(`${mixInputs}amix=inputs=${audioInputs.length + 1}:duration=longest:dropout_transition=0,volume=2.0[aout]`);
