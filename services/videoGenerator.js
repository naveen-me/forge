...
export class VideoGenerator {
  ...
  async generateVideo(questionIds, presetId, { onProgress, voiceName, profileId, customName, setId } = {}) { 
    ...
    try {
      ...
      const questions = this.loadQuestions(questionIds, { voiceKey, setId });
      ...
      const perQuestionDurations = questions.map(q => {
        const optionsCount = q.options?.length || 4;
        let total = animConfig.question_display_duration;
        if (animConfig.options_display_mode === 'one_by_one') {
          total += optionsCount * animConfig.option_reveal_delay;
        } else {
          total += animConfig.transition_duration;
        }
        total += timerDuration;
        total += animConfig.answer_reveal_duration;
        return total;
      });
      ...
      const questionsDuration = perQuestionDurations.reduce((a, b) => a + b, 0);
      ...
      const htmlContent = this.createRenderPage(preset, questions, baseUrl, totalDuration);
   ...
   */
  loadQuestions(questionIds, { voiceKey, setId } = {}) {
   ...
   */
  createRenderPage(preset, questions, baseUrl = 'http://localhost:3001', expectedTotalDurationSeconds = null) {
...
<body>
  ...
  <script type="module">
    ...
    async function renderQuestion(index) {
      ...
      if (ttsEnabled) {
        try {
          ...
          if (presetConfig?.animation?.options_display_mode === 'one_by_one') {
            const delay = Number(presetConfig?.animation?.option_reveal_delay || 0);
      ...
      // Calculate duration for this question using renderer's getTotalDuration
      const questionDuration = renderer.getTotalDuration();
  ...
  async muxTTSAudioIntoVideo({ videoPath, preset, questions, totalDurationSeconds, customName }) {
    ...
    // - question audio starts immediately when the question scene starts
    // - option audio plays sequentially after question audio completes
    // - in one_by_one mode, each option is delayed by option_reveal_delay before speaking
    ...
    const optionRevealDelay = Number(anim.option_reveal_delay || 0);

    const segments = [];
    let cursor = Math.max(0, introDuration);

    for (const q of questions) {
      // Question starts at "cursor" (scene start). Audio playback is sequential within the scene.
      let t = cursor;

      const qAudio = q?.tts_audio?.question;
      if (qAudio) {
        segments.push({ url: qAudio, startSeconds: t });
        const d = await this.getAudioDurationSeconds(qAudio);
        t += d;
      }
      ...
      if (optionsMode === 'one_by_one') {
        for (let i = 0; i < optionsCount; i++) {
          t += Math.max(0, optionRevealDelay);
          const oAudio = opts[i];
          if (oAudio) {
      ...
      // Advance cursor by the visual scene duration (not by audio duration)
      // This matches how the page waits for renderer.getTotalDuration().
      // We already computed these per-question durations earlier when computing totalDuration.
      const qAnim = preset?.config?.animation || {};
      const timerDuration = preset?.config?.timer?.duration || 5;
      const questionDisplayDuration = Number(qAnim.question_display_duration || 2);
      const transitionDuration = Number(qAnim.transition_duration || 0.3);
      const answerRevealDuration = Number(qAnim.answer_reveal_duration || 3);

      let sceneDuration = questionDisplayDuration;
      if (optionsMode === 'one_by_one') {
        sceneDuration += (q?.options?.length || 0) * Math.max(0, optionRevealDelay);
      } else {
        sceneDuration += transitionDuration;
      }
      sceneDuration += timerDuration;
      sceneDuration += answerRevealDuration;
