<template>
  <div class="pin-login-container min-vh-100 d-flex align-items-center">
    <div class="container">
...
export default {
  ...
  setup() {
    ...
    const onSubmit = async () => {
      ...
      try {
        const storedPin = await authService.getPin()
        if (storedPin && storedPin === pin.value) {
          const response = await authService.validatePin(pin.value)

          if (response.success) {
            // Show success toast
            if (window.showToast) {
              window.showToast('PIN validated successfully!', 'success');
            }
            nextTick(() => {
            ...
            // Show error toast
            if (window.showToast) {
              window.showToast(response.message || 'Invalid PIN', 'danger');
            } else {
              alert(response.message || 'Invalid PIN')
            }
          }
        } else {
          alert('Invalid PIN')
        ...
        if (window.showToast) {
          window.showToast(`PIN validation failed: ${error.message || 'An error occurred during PIN validation'}`, 'danger');
        } else {
