...
export class CanvasRenderer {
  ...
  getAnimationPhase(time) {
    ...
    if (anim.options_display_mode === 'one_by_one') {
      optionsEndTime = currentTime + (optionsCount * anim.option_reveal_delay);
    } else {
      optionsEndTime = currentTime + anim.transition_duration;
    }
    ...
    if (time < optionsEndTime) {
      ...
      if (anim.options_display_mode === 'one_by_one') {
        visibleOptions = Math.min(optionsCount, Math.floor(phaseTime / anim.option_reveal_delay) + 1);
      }
      
      return {
        phase: 'options',
        phaseTime: phaseTime,
        progress: phaseTime / (optionsEndTime - currentTime),
